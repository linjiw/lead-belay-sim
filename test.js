...
function runCase(name, params) {
  const result = simulate(params, { duration: 4.8, dt: 1/300 });
  return { name, metrics: result.metrics };
}

function clonePreset(key) {
  return { ...defaultParams(), ...presets[key].values };
}
...
assert(byName.firstClipOff.lowestPoint < byName.firstClipOn.lowestPoint, 'No first clip should increase low-point risk');

console.log('Sanity tests passed.');
for (const r of results) {
  console.log(`${r.name}: peak=${(r.metrics.maxClimberForce/1000).toFixed(2)}kN anchor=${(r.metrics.maxAnchorLoad/1000).toFixed(2)}kN low=${r.metrics.lowestPoint.toFixed(2)}m FFa=${r.metrics.actualFF.toFixed(2)} lift=${r.metrics.belayerLift.toFixed(2)}m`);
}
